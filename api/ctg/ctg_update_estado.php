...
try {
    ...
    // Obtener el estado actual del CTG para el log de auditoría
    $sql_old_estado = 'SELECT estado_id FROM ctg WHERE id = :ctg_id LIMIT 1';
    $stmt_old_estado = $db->prepare($sql_old_estado);
    $stmt_old_estado->execute([':ctg_id' => $ctgId]);
    ...
    if ($stmt_update->rowCount() > 0) {
        ...
        log_audit_action($db, 'UPDATE_CTG_STATUS', $authenticated_user['id'], 'responsable', 'ctg', $ctgId, ['old_estado_id' => $old_estado_id, 'new_estado_id' => $estadoId]); // Log de auditoría
    } else {
