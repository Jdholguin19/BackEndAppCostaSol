...
function log_message(string $message): void {
    ...
    // Imprimir en pantalla en tiempo real.
    echo $log_entry;
    ob_flush();
    flush();
 ...
 * - Si el número empieza con '+593', lo reemplaza por '0' y limpia caracteres no numéricos.
 * - Para otros casos, simplemente elimina cualquier caracter que no sea un dígito.
...
do {
    ...
    foreach ($records as $record) {
        ...
        // Limpieza de la cédula: eliminar cualquier caracter que no sea un dígito.
        $cedula = preg_replace('/[^0-9]/', '', trim($record['Identificacion'] ?? ''));

        // Si un registro no tiene cédula después de la limpieza, se omite.
        if (empty($cedula)) {
        ...
        try {
            ...
            if (!$user_id) {
                ...
                if (!empty($record['Nombre_de_Cliente'])) {
                    $parts = explode(' ', trim($record['Nombre_de_Cliente']), 2);
                ...
                if (!empty($record['Ruc_Actualizado'][0]['Content'])) $new_user_data['kissflow_ruc_url'] = $record['Ruc_Actualizado'][0]['Content'];
                if (!empty($record['Rev_Gerencia'])) $new_user_data['kissflow_rev_gerencia'] = trim($record['Rev_Gerencia']);
                if (!empty($record['Numero_de_Convenio'])) $new_user_data['kissflow_convenio'] = trim($record['Numero_de_Convenio']);
                ...
                if (!empty($record['Ruc_Actualizado'][0]['Content'])) $update_fields['kissflow_ruc_url'] = $record['Ruc_Actualizado'][0]['Content'];
                if (!empty($record['Rev_Gerencia'])) $update_fields['kissflow_rev_gerencia'] = trim($record['Rev_Gerencia']);
                if (!empty($record['Numero_de_Convenio'])) $update_fields['kissflow_convenio'] = trim($record['Numero_de_Convenio']);
            ...
            $prop_data_to_update['etapa_id'] = get_or_create_master_id($conn, 'etapa_construccion', 'nombre', $record['nometapa'] ?? null);
            ...
            if (!empty($record['fecha_entrega'])) $prop_data_to_update['fecha_entrega'] = $record['fecha_entrega'];
