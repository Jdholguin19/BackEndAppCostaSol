    ...
    if (isset($_FILES['archivo']) && $_FILES['archivo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (is_writable($uploadDir)) {
            ...
            if (move_uploaded_file($_FILES['archivo']['tmp_name'], $dest)) {
                $urlAdjunto = "https://app.costasol.com.ec/ImagenesPQR_respuestas/" . rawurlencode($name);
            } else {
    ...
    if ($is_responsable) { // Si el que responde es un responsable, notificar al cliente
        ...
        if ($cliente_id) {
            ...
            if ($oneSignalPlayerId) {
                $message_title = "Nueva respuesta a tu PQR";
                $message_body = strlen($mensaje) > 100 ? substr($mensaje, 0, 97) . '...' : $mensaje;
                send_one_signal_notification($message_title, $message_body, $oneSignalPlayerId, ['pqr_id' => $pqrId]);
            }
        }
    } else { // Si el que responde es un cliente, notificar al responsable
        $sql_get_resp_id = 'SELECT responsable_id FROM pqr WHERE id = :pqr_id LIMIT 1';
        $stmt_get_resp_id = $db->prepare($sql_get_resp_id);
        $stmt_get_resp_id->execute([':pqr_id' => $pqrId]);
