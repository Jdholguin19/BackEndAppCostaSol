...
try{
    ...
    // Todos los PQR se asignan al responsable con ID 2
    $respId = 2;
    ...
    /* ---------- 6. insertar ---------- */
    $sql = 'INSERT INTO pqr
            (numero_solicitud,id_usuario,id_propiedad,tipo_id,estado_id,
             descripcion,url_problema,responsable_id,fecha_compromiso)
            VALUES
            (:num,:uid,:pid,:tipo,1,          /* 1 = Ingresado */
             :des,:url,:resp,DATE_ADD(NOW(),INTERVAL 5 DAY))'; //
    ...
    $stmt_tipo_pqr_nombre = $db->prepare($sql_tipo_pqr_nombre);
    $stmt_tipo_pqr_nombre->execute([':tipo_id' => $tipo]);
    $tipoTicket = $stmt_tipo_pqr_nombre->fetchColumn();
