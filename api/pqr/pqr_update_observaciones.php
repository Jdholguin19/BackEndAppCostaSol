...
try{
    ...
    // Obtener las observaciones actuales del PQR para el log de auditoría
    $sql_old_obs = 'SELECT observaciones FROM pqr WHERE id = :pqr_id LIMIT 1';
    $stmt_old_obs = $db->prepare($sql_old_obs);
    $stmt_old_obs->execute([':pqr_id' => $pqrId]);
    ...
    if ($result) {
        log_audit_action($db, 'UPDATE_PQR_OBSERVATION', $authenticated_user['id'], 'responsable', 'pqr', $pqrId, ['old_observaciones' => $old_observaciones, 'new_observaciones' => $observaciones]); // Log de auditoría
        echo json_encode([
            'ok' => true, 
