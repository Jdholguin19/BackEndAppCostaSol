...
<head>
    ...
    <title>Crear CTG para Cliente - CostaSol</title>
    ...
    <style>
        ...
        .ctg-form-select:hover {
            border-color: #007bff;
        }
        
        .ctg-form-select:focus {
            outline: none;
            border-color: #007bff;
            box-shadow: 0 0 0 3px rgba(0, 123, 255, 0.15);
        }
        
        .ctg-form-select option {
            padding: 12px;
            font-size: 15px;
        }
        
        .ctg-form-select option:hover {
            background-color: #f8f9fa;
        }
        
        /* Estilo para el estado disabled */
        .ctg-form-select:disabled {
...
<div class="ctg-container">
  <div class="ctg-header">
    <h1 class="ctg-header-title">Crear CTG para Cliente</h1>
  ...
  <form id="frmCtg" enctype="multipart/form-data" class="ctg-form needs-validation" novalidate>
    <!-- cliente -->
    <label class="ctg-form-label">Cliente</label>
    <div class="search-container">
        <input type="text" id="userSearch" class="ctg-form-input" placeholder="Escriba el nombre del cliente..." autocomplete="off" required>
    ...
    <select id="propSel" name="id_propiedad" class="ctg-form-select" required>
        <option value="">Seleccione una propiedad...</option>
    </select>

    <!-- tipo -->
    <label class="ctg-form-label">Tipo de Contingencia</label>
    <select id="tipoSel" name="tipo_id" class="ctg-form-select" required>
        <option value="">Seleccione un tipo...</option>
    </select>

    <!-- descripción -->
    <label class="ctg-form-label">Descripción</label>
    <textarea id="txtDescripcion" name="descripcion" rows="4" class="ctg-form-textarea"
              placeholder="Describa detalladamente el problema o contingencia..." required></textarea>

    <!-- adjunto -->
    <label class="ctg-form-label">Adjunto (opcional)</label>
    <div class="ctg-file-input-wrapper">
        <input type="file" id="fileArchivo" name="archivo" accept="image/*,video/*,audio/*,.pdf,.doc,.docx" class="ctg-file-input">
    </div>

    <div class="ctg-button-group">
      <button type="button" class="ctg-btn-cancel" onclick="history.back()">Cancelar</button>
      <button class="ctg-btn-submit" type="submit" id="btnOk">Crear Contingencia</button>
    </div>
  </form>
</div>
...
?>
    ...
    <script>
        ...
        const ctgToken = localStorage.getItem('cs_token');
        ...
        if (!ctgToken) {
            ...
            if (container) {
                container.innerHTML = '<div class="alert alert-warning">Debes iniciar sesión para crear un CTG.</div>';
            }
        } else {
            // Cargar tipos de contingencia
            fetch('../../api/ctg/tipo_ctg.php')
            .then(r => r.json())
            .then(d => {
                if (!d.ok) return;
                d.items.forEach(t => {
        ...
        function searchClients(query) {
            ...
            fetch(`../../api/filtro_propiedad/buscar_clientes.php?q=${encodeURIComponent(query)}&limit=10`, {
                headers: { 'Authorization': `Bearer ${ctgToken}` }
            })
            .then(r => r.json())
            .then(d => {
        ...
        document.getElementById('frmCtg').addEventListener('submit', e => {
            ...
            .then(d => {
                if (d.ok) {
                    alert(`Contingencia creada exitosamente. Número: ${d.numero}`);
                    location.href = 'ctg.php';
                } else {
                    alert(d.msg || 'No se pudo crear la contingencia');
                    btnOk.disabled = false;
                    btnOk.textContent = 'Crear Contingencia';
                }
            })
            .catch(() => {
